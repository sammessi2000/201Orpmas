...
class AdminPictureController extends AdminAppController {

    public $uses = array('Picture', 'Admin.Category', 'Node', 'CategoryLinked');
    public $type = 'picture';
    public $tbl = 'Picture';
    public $form_title = 'Hình ảnh';
    ...
    public function picture_edit($id = null) {
        if ($this->data) {
            $data = $this->data[$this->tbl];
            $data_node = $this->data['Node'];
    ...
    public function picture_copy($node_id)
    {
        $this->picture_edit($node_id);
    }
}
