<?php 
$title = $lang != 'vi' ? 'title_' . $lang : 'title'; 
$description = $lang != 'vi' ? 'description_' . $lang : 'description'; 
$content = $lang != 'vi' ? 'content_' . $lang : 'content'; 
?>

<div class="navbar margin-none">
    <div class="navbar-inner radius-none">
        <a href="#" class="brand">Sửa bài viết</a>
    </div>
    <?php if($multiple_lang == true) : ?>
        <div class="lang">
            <a href="<?php echo $url_here; ?>?lang=vi" <?php if($lang == 'vi') echo 'class="active"'; ?>><i class="icon icon-vi"></i> Tiếng Việt</a>
            <a href="<?php echo $url_here; ?>?lang=en" <?php if($lang == 'en') echo 'class="active"'; ?>><i class="icon icon-en"></i> Tiếng Anh</a>
        </div>
    <?php endif; ?>
</div>

<div class="clearfix"></div>

<div class="well bg-white radius-none">
    <?php echo $this->Session->flash(); ?>
    <form action="" method="post" class="form-horizontal">
        <div class="control-group">
            <div class="span1 text-bold">Tiêu đề </div>
            <div class="span10">
                <?php echo $this->Form->hidden('Node.title'); ?>
                <?php echo $this->Form->input('Node.'.$title, array('label'=>false, 'div'=>false, 'placeholder'=>'Tiêu đề bài viết', 'class'=>'span10 news_title', 'required')); ?>
                <div class="clearfix"></div>
                <div class="msg" id="response_msg"></div>
            </div>
        </div>
        
        <div class="control-group">
            <div class="span1 text-bold">Trạng thái </div>
            <div class="span2">
                <?php echo $this->Form->input('Node.status', array('label'=>false, 'div'=>false, 'type'=>'select', 'class'=>'span12', 'options'=>array('1'=>'Xuất bản', '0'=>'Tạm lưu'), 'value'=>1)); ?>
            </div>
        </div>

        <div class="control-group">
            <div class="span1 text-bold">Thumbnail </div>
            <div class="span9">
                <?php echo $this->Form->input('Page.image', array('label' => false, 'div' => false, 'class' => 'span6', 'id' => 'thumbnail')); ?>
                <input type="button" class="btn btn-info" onclick="file_manager('thumbnail');" value="Chọn ảnh">
            </div>
        </div>
        
        <div class="control-group">
            <div class="span1 text-bold">Tóm tắt </div>
            <div class="span9">
                <?php echo $this->Form->input('Page.'.$description, array('type'=>'textarea', 'div'=>false, 'label'=>false, 'class'=>'span11', 'rows'=>3, 'required'=>"required", 'id'=>"NewsDescription")); ?>
            </div>
        </div>
        
        <div class="control-group">
            <div class="span1 text-bold">Nội dung </div>
            <div class="span10">
            <?php
                $CKEditor = new CKEditor();
                $CKEditor->config['width'] = '750';
                $CKEditor->config['height'] = '300';
                CKFinder::SetupCKEditor( $CKEditor ) ;

                $initialValue = $this->data['Page'][$content];
                echo $CKEditor->editor("data[Page][".$content."]", $initialValue, "extra");
            ?>
            </div>
        </div>    

<?php /*if($this->data['Page']['id'] == 2) : ?>
        <script type="text/javascript">
        var ckconfig = CKEDITOR.instances["data[Page][content]"];
            ckconfig.config.startupMode = 'source';
            ckconfig.config.entities = true;
            ckconfig.config.basicEntities = true;
            ckconfig.config.entities_greek = true;
            ckconfig.config.entities_latin = true;
        </script>
<?php endif;*/ ?>
        
        
        <!-- <div class="control-group">
            <div class="span1 text-bold">Seo Title </div>
            <div class="span10">
                <?php echo $this->Form->input('Page.seo_title', array('type'=>'text', 'div'=>false, 'label'=>false, 'class'=>'span10')); ?>
            </div>
        </div>
        
        <div class="control-group">
            <div class="span1 text-bold">Seo Keyword </div>
            <div class="span10">
                <?php echo $this->Form->input('Page.seo_keyword', array('type'=>'textarea', 'maxlength'=>120, 'div'=>false, 'label'=>false, 'class'=>'span10 news_keyword', 'rows'=>3)); ?>
            </div>
        </div>

        <div class="control-group">
            <div class="span1 text-bold">Seo Description </div>
            <div class="span10">
                <?php echo $this->Form->input('Page.seo_description', array('type'=>'textarea', 'maxlength'=>160, 'div'=>false, 'label'=>false, 'class'=>'span10 news_description', 'rows'=>3)); ?>
            </div>
        </div>  
        
        <div class="control-group">
            <div class="span1 text-bold">Focus Keywords </div>
            <div class="span9">
                <?php echo $this->Form->input('Page.focus_keyword', array('type'=>'text', 'maxlength'=>120, 'div'=>false, 'label'=>false, 'class'=>'span3 forcus_key',  'id'=>"NewsFocusKeyword")); ?>
                <span class="SeoStatus">
                    <span>Thông tin chuẩn SEO : </span>
                    <span class="SeoStatusTitle">Tiêu đề</span>,
                    <span class="SeoStatusDescription  text-center">Mô tả</span>,
                    <span class="SeoStatusContent  text-center">Nội dung</span>
                </span>
            </div>
        </div>   -->
        
        <div class="control-group">
            <div class="span8 offset1">
                <input type="submit" name="submit" value="Hoàn tất" class="btn btn-primary" />
                <input type="reset" name="reset" value="Làm lại" class="btn btn-info" />
            </div>
        </div>  
    </form>
</div>

<script type="text/javascript">
    $('.forcus_key').keyup(function () {
        check();
    });

    function check()
    {
        var v = $('.forcus_key').val();
        var arr = new Array();
        arr = v.split(' ');
        if (arr.length < 2)
            return false;

        var title = $('.news_title').val();
        var des = $('.news_description').val();
        var content = CKEDITOR.instances["data[Page][content]"].getData();

        if (searchText(v, title) == 1)
            $('.SeoStatusTitle').addClass('valid-seo');
        else
            $('.SeoStatusTitle').removeClass('valid-seo');


        if (searchText(v, des) == 1)
            $('.SeoStatusDescription').addClass('valid-seo');
        else
            $('.SeoStatusDescription').removeClass('valid-seo');


        if (searchText(v, content) == 1)
            $('.SeoStatusContent').addClass('valid-seo');
        else
            $('.SeoStatusContent').removeClass('valid-seo');
    }

    function searchText(key, text)
    {
        if (key.length <= 0 || text.length <= 0)
            return 0;

        var regExp = new RegExp(key, 'i');

        if (regExp.test(text))
        {
            return 1;
        }

        return 0;
    }

    //Kiểm tra độ dài text
    $('.news_title').maxlength({
        alwaysShow: true,
        threshold: 10,
        warningClass: "label label-info",
        limitReachedClass: "label label-warning",
        placement: 'top',
        message: 'Đã dùng %charsTyped% / %charsTotal% ký tự.'
    });

    $('.news_description').maxlength({
        alwaysShow: true,
        threshold: 10,
        warningClass: "label label-info",
        limitReachedClass: "label label-warning",
        placement: 'top',
        message: 'Đã dùng %charsTyped% / %charsTotal% ký tự.'
    });

    $('.news_keyword').maxlength({
        alwaysShow: true,
        threshold: 10,
        warningClass: "label label-info",
        limitReachedClass: "label label-warning",
        placement: 'top',
        message: 'Đã dùng %charsTyped% / %charsTotal% ký tự.'
    });


    //Kiểm tra trùng lặp tiêu đề
    var is_sendding_request = false;

    setInterval(function() {
        check();
        if (is_sendding_request == false)
        {
            var v = $('.news_title').val();
            var n = v.length;
            var url = "<?php echo DOMAINAD; ?>admin_node/check_exits_news/" + v + "/" + <?php echo $this->data['Node']['id'] ?>;

            if (n >= 4)
            {
                is_sendding_request = true;
                $.ajax({
                    url: url,
                    cache: false,
                    dataType: "html",
                    success: function (data) {
                        if (data == "1")
                            $('#response_msg').html("<div style='color: red; font-weight: bold; margin-top: 15px;'>Tiêu đề đã tồn tại !!!</div>");
                        else
                            $('#response_msg').html("");
                        
                        is_sendding_request = false;
                    },
                    error: function () {
                        is_sendding_request = false;
                    }
                });
            }
            
            return false;
        }
    }, 3000);
</script>