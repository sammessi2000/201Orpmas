...
<tr class="text-bold warning">
	<td>Gói cước</td>
	<?php foreach($this->data['ram'] as $c) { ?>
	<td><?php echo $c; ?></td>
	<?php } ?>
</tr>
...
<tr>
	...
	<?php $i=-1; foreach($this->data['ram'] as $c) { $i++; ?>
	<td>
		<?php 
			$checked = ''; 
			if(	isset($this->data['detail']['server'][$v['Server']['id']]['ram']) 
				&& is_array($this->data['detail']['server'][$v['Server']['id']]['ram'])
				&& in_array($c, $this->data['detail']['server'][$v['Server']['id']]['ram'])
			) 
...
<?php foreach($this->data['cloud'] as $v) { ?>
	<?php $i=-1; foreach($this->data['ram'] as $c) { $i++; ?>
		<?php 
			$checked = ''; 
			if(	isset($this->data['detail']['cloud'][$v['Cloud']['id']]['ram']) 
				&& is_array($this->data['detail']['cloud'][$v['Cloud']['id']]['ram'])
				&& in_array($c, $this->data['detail']['cloud'][$v['Cloud']['id']]['ram'])
			)
			{ 
				$checked = 'checked="checked"';
		?>
